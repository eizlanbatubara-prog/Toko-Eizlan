...
<body>
    ...
    <script>
        ...
        function checkout() {
            ...
            let message = '*PESANAN BARU - TOKO EIZLAN*%0A%0A';
            message += '*Detail Pesanan:*%0A';
            
            cart.forEach(item => {
                message += `${item.emoji} ${item.name}%0A`;
                message += `   ${item.quantity} x ${formatRupiah(item.price)} = ${formatRupiah(item.price * item.quantity)}%0A`;
            });
            ...
            message += 'Terima kasih telah berbelanja! 🛍️';

            // Ganti 628123456789 dengan nomor WA toko Anda (format: 628xxx)
